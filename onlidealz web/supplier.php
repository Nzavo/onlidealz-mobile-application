...
if (empty($_SESSION['user_id'])) {
    ...
    if (isset($_GET['inid'])) {
        $id = $_GET['inid'];
        $status = 2;
        mysqli_query($db, "UPDATE supplier set status='$status' where reg_id='$id'");
    ...
    if (isset($_GET['idi'])) {
        $idi = $_GET['idi'];
        $status = 1;
        mysqli_query($db, "UPDATE supplier set status='$status' where reg_id='$idi'");
        header('location:supplier');
    }
?>
<!DOCTYPE html>
                                ...
                                <div class="table-responsive m-t-40">
                                    <table id="myTable" class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            ...
                                            <?php
                                                $sql = "SELECT * FROM supplier order by reg_id desc";
                                                $query = mysqli_query($db, $sql);

                                                if (!mysqli_num_rows($query) > 0) {
                                                    echo '<td colspan="12"><center>No Record was Found</center></td>';
                                                } else {
                                                ...
                                                <td><?php echo $rows['business_registry']; ?></td>
                                                <td><?php echo $rows['existence']; ?></td>
                                                ...
                                                <td class="center">
                                                    ...
                                                    <a href="supplier?inid=<?php echo htmlentities($rows['reg_id']); ?>"
                                                    ...
                                                    <a href="supplier?idi=<?php echo htmlentities($rows['reg_id']); ?>"
                                                    ...
                                                    <a href="supplier?inid=<?php echo htmlentities($rows['reg_id']); ?>"
                                                        ...
                                                        <button class="btn btn-danger"> Block</button></a>
                                                    <?php } else { ?>
                                                    <a href="supplier?id=<?php echo htmlentities($rows['reg_id']); ?>"
                                                        onclick="return confirm('Are you sure you want to allow this blocked supplier?');"><button
