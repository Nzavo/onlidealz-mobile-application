...
if (empty($_SESSION['user_id'])) {
    ...
    if (isset($_GET['inid'])) {
        $id = $_GET['inid'];
        $status = 2;
        mysqli_query($db, "UPDATE customer set status='$status' where reg_id='$id'");
    ...
    if (isset($_GET['idi'])) {
        $idi = $_GET['idi'];
        $status = 1;
        mysqli_query($db, "UPDATE customer set status='$status' where reg_id='$idi'");
        header('location:customer');
    }
?>
                                ...
                                <div class="table-responsive m-t-40">
                                    <table id="myTable" class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            ...
                                            <?php
                                                $sql = "SELECT * FROM customer order by reg_id desc";
                                                ...
                                                <td class="center">
                                                    ...
                                                    <a href="customer?inid=<?php echo htmlentities($rows['reg_id']); ?>"
                                                    ...
                                                    <a href="customer?idi=<?php echo htmlentities($rows['reg_id']); ?>"
                                                        onclick="return confirm('Are you sure you want to Accept this Customer?');">
                                                        <button class="btn btn-success"> Accepted</button></a>
                                                    <a href="customer?inid=<?php echo htmlentities($rows['reg_id']); ?>"
                                                        onclick="return confirm('Are you sure you want to block this Customer?');">
                                                        <button class="btn btn-danger"> Block</button></a>
                                                    <?php } else { ?>
                                                    <a href="customer?id=<?php echo htmlentities($rows['reg_id']); ?>"
                                                        onclick="return confirm('Are you sure you want to allow this blocked Customer?');"><button
