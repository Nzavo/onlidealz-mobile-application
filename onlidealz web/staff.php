...
if (empty($_SESSION['user_id'])) {
    ...
    if (isset($_GET['inid'])) {
        $id = $_GET['inid'];
        $status = 2;
        mysqli_query($db, "UPDATE staff set status='$status' where reg_id='$id'");
    ...
    if (isset($_GET['idi'])) {
        $idi = $_GET['idi'];
        $status = 1;
        mysqli_query($db, "UPDATE staff set status='$status' where reg_id='$idi'");
        header('location:staff');
    }
?>
<!DOCTYPE html>
        ...
        <div class="page-wrapper bg-danger" style="height:100%">
            <div class="row page-titles">
                <div class="col-md-5 align-self-center">
                    <h3 class="text-primary">Manage Staff</h3>
                </div>
                                ...
                                <div class="table-responsive m-t-40">
                                    <table id="myTable" class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            ...
                                            <?php
                                                $sql = "SELECT * FROM staff order by reg_id desc";
                                                ...
                                                <td class="center">
                                                    ...
                                                    <a href="staff?inid=<?php echo htmlentities($rows['reg_id']); ?>"
                                                    ...
                                                    <a href="staff?idi=<?php echo htmlentities($rows['reg_id']); ?>"
                                                        onclick="return confirm('Are you sure you want to Accept this staff?');">
                                                        <button class="btn btn-success"> Accepted</button></a>
                                                    <a href="staff?inid=<?php echo htmlentities($rows['reg_id']); ?>"
                                                        onclick="return confirm('Are you sure you want to block this staff?');">
                                                        <button class="btn btn-danger"> Block</button></a>
                                                    <?php } else { ?>
                                                    <a href="staff?id=<?php echo htmlentities($rows['reg_id']); ?>"
                                                        onclick="return confirm('Are you sure you want to allow this blocked staff?');"><button
